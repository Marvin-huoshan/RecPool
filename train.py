...
def train(dataset, model, args, index, same_feat=True, val_dataset=None, test_dataset=None, writer=None,
    ...
    #对所有requires_grad的参数进行优化
    #filter(A,B)对所有B中的元素应用A进行判断
    ...
    for epoch in range(args.num_epochs):
        ...
        if writer is not None:
            ...
            writer.add_scalar('acc/val_acc'+'_'+str(index), val_result['acc'], epoch)
    ...
    plt.savefig(gen_train_plt_name(args), dpi=1200)
    plt.close()
...
def prepare_data(graphs, args, test_graphs=None, max_nodes=0):
    ...
    # minibatch
    '''
        一种十分方便的数据读取机制，即使用torch.utils.data.Dataset与Dataloader组合得到数据迭代器。
        在每次训练时，利用这个迭代器输出每一个batch数据，并能在输出时对数据进行相应的预处理或数据增广操作
    '''
...
def syn_community1v2(args, writer=None, export_graphs=False):
    ...
    # data
    #datagen->gen/data.py; featgen->gen/feat.py
    #生成500个BA图
    ...
    #input_dim->特征维度
    #args.hidden_dim=20; args.output_dim=20; args.num_gc_layers=3
    else:
        print('Method: base')
...
def benchmark_task_val(args, writer=None, feat='node-label'):
    ...
    for i in range(10):
        ...
        _, val_accs = train(train_dataset, model, args, i, val_dataset=val_dataset, test_dataset=None,
            writer=writer)
...
def main():
    ...
    if os.path.isdir(path):
        ...
        #将文件夹目录删除
        shutil.rmtree(path)
    #用来记录训练过程中的数据，与tensorboard配合进行可视化
